...
green='\e[0;32m'
###############
CACHE="/etc/julak/ipcache"
TMP="/tmp/ipcache.$$"

REPO1="https://raw.githubusercontent.com/julakhss/opt/main/ip"
REPO2="https://raw.githubusercontent.com/julakhss/opt/main/ip"
...
if [ ! -f "$CACHE" ]; then
curl -s --max-time 10 $REPO1 -o "$TMP"

if [ ! -s "$TMP" ]; then
curl -s --max-time 10 $REPO2 -o "$TMP"
fi
...
echo "   IP VPS Tidak Terdaftar"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━"
exit
fi

# hitung masa aktif
if [[ "$valid" =~ [Ll]ifetime ]]; then
...
# Repository Github
jamban="https://raw.githubusercontent.com/minionstore/c/main/"
...
#install cf
wget ${jamban}service/cf.sh && chmod +x cf.sh && ./cf.sh
rm -f /root/cf.sh
clear
else
...
function pasang_domain() {
    ...
    mkdir -p /etc/trojan
    mkdir -p /etc/ssh
    mkdir -p /opt/julak/data
    touch /etc/vmess/.vmess.db
    touch /etc/vless/.vless.db
    touch /etc/trojan/.trojan.db
    touch /opt/julak/data/ssh.db
    touch /etc/bot/.bot.db
    echo "& plughin Account" >>/etc/vmess/.vmess.db
    echo "& plughin Account" >>/etc/vless/.vless.db
    echo "& plughin Account" >>/etc/trojan/.trojan.db
    echo "& plughin Account" >>/opt/julak/data/ssh.db
...
wget -q -O /etc/info.txt "${jamban}port.txt"
...
curl -sS ${jamban}service/password | openssl aes-256-cbc -d -a -pass pass:scvps07gg -pbkdf2 > /etc/pam.d/common-password
...
curl ${jamban}config/nginx.conf > /etc/nginx/nginx.conf
...
# install badvpn
cd
wget -O /usr/sbin/badvpn "${jamban}service/badvpn" >/dev/null 2>&1
chmod +x /usr/sbin/badvpn > /dev/null 2>&1
wget -q -O /etc/systemd/system/badvpn1.service "${jamban}service/badvpn1.service" >/dev/null 2>&1
wget -q -O /etc/systemd/system/badvpn2.service "${jamban}service/badvpn2.service" >/dev/null 2>&1
wget -q -O /etc/systemd/system/badvpn3.service "${jamban}service/badvpn3.service" >/dev/null 2>&1
...
wget -O /etc/default/dropbear "${jamban}service/dropbear"
...
wget -O /etc/squid/squid.conf "${jamban}config/squid3.conf"
...
wget -q ${jamban}service/bbr.sh && chmod +x bbr.sh && ./bbr.sh

wget -q ${jamban}service/ipserver && chmod +x ipserver && ./ipserver
# blokir torrent
...
# optimizer tiap 1 jam
0 * * * * root /usr/local/sbin/clearlog
END
...
wget -q -O /etc/nginx/conf.d/xray.conf "${jamban}config/xray.conf"
wget -q -O /etc/haproxy/haproxy.cfg "${jamban}config/haproxy.cfg"
...
# LIMIT QUOTA
cd
wget -q -O /usr/local/sbin/quota "${jamban}service/quota"
chmod +x /usr/local/sbin/quota
cd /usr/local/sbin/
sed -i 's/\r//' quota
clear
#SERVICE LIMIT QUOTA VMESS
...
wget -q -O /etc/websocket/ws.py "${jamban}/service/ws.py"
...
wget -q -O /usr/local/bin/ws-ovpn ${jamban}service/ws-ovpn.py
...
insbkp() {
wget -q ${jamban}service/installbckp.sh && chmod +x installbckp.sh && ./installbckp.sh
clear
}

insmenu() {
wget -q ${jamban}service/up.sh && chmod +x up.sh && ./up.sh
clear
}
...
UDP="https://raw.githubusercontent.com/minionstore/c/main/config/"
# install udp-custom
